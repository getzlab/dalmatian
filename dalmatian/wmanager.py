        ...
        if isinstance(attrs, pd.DataFrame):
            ...
            for i,row in attrs.iterrows():
                attr_list.extend([{
                    'name':row.name,
                    'entityType':etype,
                    'operations': [{"op": "AddUpdateAttribute", "attributeName": i, "addUpdateAttribute": {'entityType': reserved_attrs[i], 'entityName': str(j)} if i in reserved_attrs else str(j)} for i,j in row.iteritems() if not pd.isnull(j)]
                }])
        elif isinstance(attrs, pd.Series):
            attr_list = [{
                'name':i,
                'entityType':etype,
                'operations': [{"op": "AddUpdateAttribute", "attributeName": attrs.name, "addUpdateAttribute": {'entityType': reserved_attrs[attrs.name], 'entityName': str(j)} if attrs.name in reserved_attrs else str(j)}]
